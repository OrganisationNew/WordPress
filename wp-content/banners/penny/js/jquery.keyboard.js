...
$.keyboard = function(el, options){
	...
	base.reveal = function(){
		...
		base.$keyboard.css({ position: 'absolute', left: 350, top: 300 });
	...
	base.buildKeyboard = function(){
		...
		/*container = $('<div />', { id:'keyboardmain' })
			.addClass('ui-keyboard ' + o.css.container + (o.alwaysOpen ? ' ui-keyboard-always-open' : '') )
			.attr({ 'role': 'textbox' })
			.hide();*/
			
		/*code added for Keyboard draggable */
		
		 container = $('#keyboardmain').addClass('ui-keyboard ' + o.css.container + (o.alwaysOpen ? ' ui-keyboard-always-open' : '') )
			.attr({ 'role': 'textbox' })
			.hide();
	...
	base.destroy = function() {
		...
		alert("asdasdasdasdas " + o.css.input);
		base.$el
	...
	$.keyboard.layouts = {
		...
		'international' : {
			'default': [
				...
				'a s d f g h j k l ; \' ',
				'{shift} z x c v b n m , . / {shift}',
				'{cancel} @ {alt} {space} {alt} .com {accept}'
			],
			'shift': [
				'~ ! @ # $ % ^ & * ( ) _ + {bksp}',
				'{tab} Q W E R T Y U I O P { } |',
